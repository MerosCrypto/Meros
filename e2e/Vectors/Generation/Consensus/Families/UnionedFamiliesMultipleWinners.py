...
merit.add(
  PrototypeBlock(
    ...
    packets=[
      VerificationPacket(claim.hash, list(range(4))),
      VerificationPacket(splitSend.hash, list(range(4)))
    ]
  ).finish(4, merit)
)
...
merit.add(
  PrototypeBlock(
    ...
    packets=[
      ...
      VerificationPacket(sends[3].hash, [2]),
      VerificationPacket(sends[4].hash, [3])
    ]
  ).finish(4, merit)
)
...
with open("e2e/Vectors/Consensus/Families/UnionedFamiliesMultipleWinners.json", "w") as vectors:
