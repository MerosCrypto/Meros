...
proc main() {.thread.} =
  ...
  let cachedInputs: seq[Input] = meritFollowup(functions, merit)
  let cachedTransactions: HashSet[Hash[256]] = transactionsFollowup(transactions, cachedInputs)
  consensusFollowup(merit[], consensus, cachedTransactions)
