...
static int argon_fill_memory_blocks_st(argon2_instance_t *instance) {
    uint32_t r, s, l;
...
static int argon_fill_memory_blocks_mt(argon2_instance_t *instance) {
    uint32_t r, s;
...
#if defined(ARGON2_NO_THREADS)
    return argon_fill_memory_blocks_st(instance);
#else
    return instance->threads == 1 ?
            argon_fill_memory_blocks_st(instance) : argon_fill_memory_blocks_mt(instance);
#endif
}
...
int argon_initialize(argon2_instance_t *instance, argon2_context *context) {
    ...
    /* uint8_t blockhash[ARGON2_PREHASH_SEED_LENGTH]; */
    /* Hashing all inputs */
    argon_initial_hash(blockhash, context, instance->type);
     ...
     */
    argon_fill_first_blocks(blockhash, instance);
    /* Clearing the hash */
